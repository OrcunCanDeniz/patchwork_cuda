...
void PointT2XYZI(pcl::PointCloud<PointXYZILID> &src, pcl::PointCloud<pcl::PointXYZI>::Ptr dst) {
  dst->points.clear();
...
std::vector<int> ground_classes_except_terrain = {ROAD,
                                                  PARKING,
                                                  SIDEWALKR,
                                                  OTHER_GROUND,
                                                  LANE_MARKING};
...
template <typename PointT>
void discern_ground(const pcl::PointCloud<PointT> &src,
                    pcl::PointCloud<PointT> &ground,
                    pcl::PointCloud<PointT> &non_ground) {
  // Ensure that PointT is PointT because this function only works in the
  // SemanticKITTI
  if (!std::is_same<PointT, PointXYZILID>::value) {
    throw invalid_argument(
        "This function only supports `PointT`. Not "
        "implemented for this point type.");
  }
...
template <typename PointT>
void pc2pcdfile(const pcl::PointCloud<PointT> &TP,
                const pcl::PointCloud<PointT> &FP,
                const pcl::PointCloud<PointT> &FN,
                const pcl::PointCloud<PointT> &TN,
